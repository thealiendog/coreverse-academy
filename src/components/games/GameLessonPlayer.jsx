...
export default function GameLessonPlayer() {
  ...
  const [loadingAudio,      setLoadingAudio]      = useState(false); // true from speak() start until 'playing' event
  ...
  const speak = useCallback(async (text, onDone, options = {}) => {
    ...
    try {
      ...
      const onPlaying        = () => { console.log(`[audio ${ts()}] EVT playing`); setLoadingAudio(false); };
        ...
        >
          ...
          {speaking && (
            <div style={{
              ...
              animation: loadingAudio ? 'guide-ring 0.7s ease-in-out infinite' : 'guide-ring 1.2s ease-in-out infinite',
              pointerEvents: 'none',
