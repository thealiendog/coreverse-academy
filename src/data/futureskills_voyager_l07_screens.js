      ...
      screens: [
...

        {
          ...
          vocab: [
            ...
            {
              word: `well-defined problem`,
              definition: `A problem with a clear goal, clear constraints, and a path that is mostly about finding the right solution. Distinct from ill-defined problems where the goal itself is fuzzy and the problem is partly about figuring out what the problem actually is. Different problem types call for different problem-solving approaches.`,
              audioPrompt: `A well-defined problem is one with a clear goal, clear constraints, and a path that is mostly about finding the right solution, {name}. A math homework problem is well-defined: there is a specific correct answer and the constraints are explicit. Ill-defined problems are different: the goal itself is fuzzy, the constraints unclear, and part of the work is figuring out what the problem actually is. Designing a community garden is ill-defined: what counts as "good," who it's for, and what constraints really matter all need to be worked out before you can even identify candidate solutions. This distinction matters because different problem types call for different approaches. The well-defined math problem calls for different methods than the ill-defined design challenge. Many people apply the same approach to both types, which is part of why some problems get solved poorly.`,
            },
            {
              word: `problem framing`,
              definition: `The process of defining what the problem actually is before trying to solve it. Expert problem solvers spend more time than novices on understanding and framing the problem before attacking it. A well-framed problem is much easier to solve than a poorly-framed one. Getting the framing wrong can mean solving the wrong problem efficiently.`,
              audioPrompt: `Problem framing is the process of defining what the problem actually is before trying to solve it, {name}. Expert problem solvers spend more time than novices on understanding and framing the problem before attacking it. This is one of the most consistent findings in research on expert performance dating back to George Pólya's work in 1945. A well-framed problem is much easier to solve than a poorly-framed one. Getting the framing wrong means solving the wrong problem, sometimes efficiently, which produces no real improvement. Many real-world failures involve solving a well-defined version of the wrong problem. A company invests in making a product slightly better (well-defined: what features to add) when the real problem is that the wrong people are using the product (ill-defined: who is the product for and why). Developing the habit of checking your problem framing before committing to a solution approach is one of the most valuable problem-solving skills available.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            {
              word: `accumulated convention`,
              definition: `The way things have been done historically, often without the reason being remembered or questioned. First principles thinking is valuable precisely because it questions accumulated conventions: "things are expensive because they've always been expensive" is a non-answer. Some conventions encode real wisdom; others are just historical accidents that persist.`,
              audioPrompt: `Accumulated convention is the way things have been done historically, often without the reason for the convention being remembered or questioned, {name}. First principles thinking is valuable precisely because it questions accumulated conventions. SpaceX's analysis of rocket launches found that launch prices reflected accumulated conventions in the aerospace industry rather than the actual cost of materials and engineering. The fundamentals allowed for much cheaper launches once conventional assumptions were questioned. Some accumulated conventions encode real wisdom: they persist because they actually work and alternatives have been tried and failed. Others are historical accidents that persist simply because no one questioned them. The skill is distinguishing between the two. First principles thinking often accelerates by asking "is this constraint real or just assumed?" and "what would this look like if we started fresh?" When the convention is just a convention, questioning it can unlock dramatically better solutions.`,
            },
            {
              word: `local optimum`,
              definition: `A solution that is better than all nearby alternatives but not the globally best solution available. Bottom-up empirical approaches risk getting stuck in local optima by refining the same approach without realizing a completely different approach would work far better. First principles thinking can escape local optima by questioning the approach itself rather than refining it.`,
              audioPrompt: `A local optimum is a solution that is better than all nearby alternatives but not the globally best solution available, {name}. It is a key risk in bottom-up empirical approaches. When you refine an approach based on continuous feedback, you tend to improve steadily until you hit the best version of that approach. But the best version of the wrong approach can still be much worse than a completely different approach. Bottom-up empirical work risks getting stuck in local optima because the method suggests refining based on nearby alternatives. First principles thinking can escape local optima by questioning the approach itself rather than just refining it. SpaceX questioning whether rockets needed to be expensive at all is an example of escaping a local optimum in aerospace: decades of incremental improvement on expensive rockets had found very good versions of expensive rockets, but hadn't escaped the frame of expensive rockets being the only option.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            },
            {
              ...
              definition: `An empirical approach to problem solving that centers on understanding user needs through observation and iteration, building prototypes, and testing with real users rather than solving problems theoretically. Associated with IDEO and Tim Brown. Emphasizes not knowing the fundamentals in advance and letting observation drive the design.`,
              audioPrompt: `Design thinking is an empirical approach to problem solving that centers on understanding user needs through observation and iteration, {name}. Rather than starting with a theory about what users need, design thinking emphasizes going out and observing people in real contexts, discovering needs that users themselves often can't articulate. You build prototypes, test them with real users, watch what happens, and iterate. The approach is associated with the design firm IDEO and its CEO Tim Brown. Design thinking rejects the assumption that you can know what users need by thinking about it theoretically. It treats that assumption as the fundamental error that most top-down approaches make about human-centered problems. The emphasis on observation, prototyping, and testing makes it explicitly empirical. Design thinking has been influential in product development, public policy, and organizational change because of how consistently it produces solutions that actually work for the people who use them.`,
            },
            {
              word: `Lean Startup`,
              definition: `Methodology developed by Eric Ries (2011) for building businesses empirically: build small experiments, measure user response, learn, iterate. Applied the empirical bottom-up approach to the domain of startups, where most conventional business planning had been too top-down. Has been applied across thousands of businesses since its introduction.`,
              audioPrompt: `Lean Startup is a methodology developed by Eric Ries in his 2011 book of the same name, {name}. Ries applied the empirical bottom-up approach to the domain of startups. The traditional approach to starting a business was largely top-down: write a business plan, project your assumptions, then build and launch. Ries observed that this approach produced a lot of elegantly wrong answers because the assumptions in the business plan often turned out to be wrong. His methodology replaces the big launch with small experiments: build a minimum viable product, measure real user behavior, learn what assumptions were right and wrong, and iterate based on actual evidence. The cycle is called build-measure-learn. It has been applied across thousands of businesses since 2011 with substantial success. It is also a good illustration of why bottom-up empirical approaches often outperform top-down planning in domains where fundamentals are uncertain and human behavior is involved.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            {
              word: `iterative cycle`,
              definition: `A repeating loop of generating a hypothesis or design, testing it against reality, learning from the result, and updating the approach. The core mechanism of hybrid problem solving. Real engineering, real research, and real design all work in iterative cycles rather than producing perfect solutions on the first attempt.`,
              audioPrompt: `An iterative cycle is a repeating loop of generating a hypothesis or design, testing it against reality, learning from the result, and updating the approach, {name}. It is the core mechanism of hybrid problem solving. Real engineering, research, and design all work in iterative cycles rather than producing perfect solutions on the first attempt. The first version is a starting point. You test it, observe what works and what fails, update your understanding, and generate the next version. The cycle repeats until the solution is good enough or constraints require stopping. Lean Startup builds the iterative cycle into its methodology explicitly: build, measure, learn, and repeat. Donald Schön's reflection-in-action describes the same cycle in individual practitioners. The key insight from both is that improvement comes from the loop, not from any single execution. People who treat the first attempt as the solution are skipping the learning that makes iteration so powerful.`,
            },
            {
              word: `cost structure`,
              definition: `The time and resource requirements of a problem-solving approach. Top-down is cheap (no experiments needed) but risks elegant wrong answers. Bottom-up is expensive (experiments take time and resources) but is harder to get wrong by pure theorizing. Hybrid is medium-cost but requires judgment. Understanding cost structure helps match the approach to the problem.`,
              audioPrompt: `Cost structure in problem solving refers to the time and resource requirements of a particular approach, {name}. Each of the three problem-solving approaches has a different cost structure. Top-down first-principles thinking is relatively cheap: you don't need experiments, just careful reasoning from fundamentals. The cost is the risk of elegant wrong answers when fundamentals are incorrect. Bottom-up empirical thinking is relatively expensive: experiments take time, resources, and iteration cycles. The cost is the time and resources; the benefit is being harder to get wrong through pure theorizing. Hybrid thinking is medium-cost: you need some experimentation to test your theoretical hypotheses, but less than a pure bottom-up approach. Understanding cost structure helps you match the approach to the problem. A problem with a tight deadline and few resources for experiments calls for more top-down thinking. A problem with complex human behavior and available time for testing calls for more bottom-up. Most real problems call for some judgment about how much of each.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            {
              word: `problem-solving default`,
              definition: `The approach you naturally use when you encounter problems without deliberate choice. Most people develop a default by accident through how they were taught and what they practiced. Making your default explicit allows you to choose it deliberately and recognize when a different approach would work better for the specific problem in front of you.`,
              audioPrompt: `A problem-solving default is the approach you naturally use when you encounter problems without deliberate choice, {name}. Most people develop a default by accident: through how they were taught, what types of problems they practiced, and which approaches felt natural to their cognitive style. The default shapes how they approach every new problem, including problems where a different approach would work better. Making your default explicit is the immediate value of comparing the three approaches. Once you can name what you do by default and why, you gain two things. First, you can choose it deliberately rather than falling into it unconsciously. Second, you can recognize when the problem in front of you calls for a different approach. The kids who develop this kind of deliberate self-awareness about their problem-solving style at 12 will outperform peers who never reflect on it, because they accumulate the benefits of making good approach choices across thousands of problems over decades.`,
            },
            {
              word: `context-dependent approach`,
              definition: `The alternative to committing to a single problem-solving default: being fluent in matching the approach to the specific problem type. Top-down for well-defined problems where fundamentals are known, bottom-up for complex domains with uncertain fundamentals, hybrid as the general-purpose default. The counterargument to having any single default.`,
              audioPrompt: `Context-dependent approach is the alternative to committing to a single problem-solving default, {name}. Instead of picking one approach as your default, you develop fluency in matching the approach to the specific problem type. Top-down first-principles thinking for well-defined problems where you can correctly identify the fundamentals. Bottom-up empirical thinking for complex domains where fundamentals are uncertain and experimentation is possible. Hybrid iterative thinking as the general-purpose approach for most real-world problems that fall somewhere between those extremes. The counterargument to the default-approach framing makes this case explicitly: maybe the right move is radical context-dependence rather than any default. The response to that counterargument is the central intellectual challenge of the argument builder. Both the commitment to a default and the commitment to pure context-dependence have real advantages and real weaknesses. Real expert problem solvers typically have both a preferred approach AND the flexibility to recognize when to deviate from it.`,
            },
          ],
        },
