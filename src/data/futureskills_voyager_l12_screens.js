      ...
      screens: [
...

        {
          ...
          vocab: [
            ...
            {
              word: `CSTA standards`,
              definition: `Computer Science Teachers Association standards that guide how computer science is taught in schools in the United States. Shaped significantly by Jeannette Wing's computational thinking framework. Include computational thinking skills alongside programming knowledge as core competencies for K-12 computer science education.`,
              audioPrompt: `The CSTA standards are the Computer Science Teachers Association standards that guide how computer science is taught in schools across the United States, {name}. They were shaped significantly by Jeannette Wing's computational thinking framework from her 2006 paper. The standards include computational thinking skills alongside specific programming knowledge as core competencies for K-12 computer science education. This means schools that follow the standards teach not just how to write code in specific languages but also the underlying mental framework: decomposition, pattern recognition, abstraction, algorithms, and debugging. The standards represent the educational system's recognition that computational thinking is valuable for all students, not just those who will go on to be programmers. Whether a student ends up writing code professionally or not, developing the mental framework of computational thinking is considered worthwhile by the standards.`,
            },
            {
              word: `pattern library`,
              definition: `The accumulated collection of patterns a person has recognized and stored through experience. The reason people with strong pattern recognition often appear very smart: they have a large library of patterns and the habit of looking for them. Pattern libraries grow through deliberate practice and compound over time as each new pattern makes future recognition faster.`,
              audioPrompt: `A pattern library is the accumulated collection of patterns a person has recognized and stored through experience, {name}. It explains one of the most important mechanisms behind what looks like expert intuition. People with strong pattern recognition often appear very smart. What they actually have is a large library of patterns built through years of practice and the habit of looking for patterns in new situations. The library grows through deliberate exposure to varied problems and through the habit of asking "does this structure remind me of something else?" Each new pattern recognized makes future recognition faster because the library has more to match against. Chess grandmasters have enormous pattern libraries of board configurations. Expert doctors have pattern libraries of symptoms. Expert teachers have pattern libraries of student confusion types. The library is built over years and compounds in value: each addition makes the whole more powerful. Starting to deliberately build your pattern library early, across many domains, is one of the highest-leverage investments in long-term cognitive capability.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            },
            {
              ...
              definition: `Noticing similarities and recurring elements, especially across problems that seem different on the surface. Allows transfer of learning: if two problems have the same underlying structure, the solution approach transfers. Compounds over time because each pattern recognized makes future recognition faster. Strong pattern recognition is a major component of what looks like expertise.`,
              audioPrompt: `Pattern recognition in computational thinking is noticing similarities and recurring elements, especially across problems that seem different on the surface, {name}. A programmer who notices that sorting names alphabetically and ordering dates chronologically use the same underlying logic can write one general solution for both. Someone who notices that negotiating with a parent about screen time and negotiating with a teacher about a deadline use the same conversational moves can apply what works in one to the other. Pattern recognition allows transfer of learning across contexts. The kid who notices the structure of a math word problem is similar to one they've solved before can apply the same approach. Pattern recognition compounds over time because each new pattern recognized makes future recognition faster. You have more patterns to match against. People with strong pattern recognition often look very smart. What they actually have is a large library of patterns built through practice and the deliberate habit of looking for underlying similarities.`,
            },
            {
              word: `transfer of learning`,
              definition: `Applying knowledge or a problem-solving approach from one context to a new, different-seeming context. Enabled by pattern recognition: if two problems share underlying structure, the solution from the first transfers to the second. Examples: the same conversational moves used to negotiate screen time also work for negotiating assignment deadlines; braising chicken and braising tofu use similar techniques.`,
              audioPrompt: `Transfer of learning is applying knowledge or a problem-solving approach from one context to a new, different-seeming context, {name}. It's one of the most powerful outcomes of strong pattern recognition. If you can spot that two problems share underlying structure even though they look different on the surface, you can transfer what you already know. The programmer who notices that sorting names alphabetically and ordering dates chronologically use the same logic can write one general solution for both, rather than solving each from scratch. The cook who notices that braising chicken and braising tofu use similar techniques can adapt recipes without starting over. The student who notices that negotiating with a parent about screen time and negotiating with a teacher about a deadline use the same conversational moves can apply what works in one situation to the other. Transfer of learning is what makes a large pattern library so valuable. Each new pattern recognized doesn't just help you in that exact context. It gives you a tool that transfers to every future context with the same underlying structure.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            {
              word: `algorithm`,
              definition: `A step-by-step procedure for solving a problem. Recipes, long division procedures, furniture assembly instructions, and navigation directions are all algorithms. Programmers write algorithms in code, but algorithms exist in many non-programming contexts. Making an algorithm explicit makes problem-solving teachable, automatable, and evaluable.`,
              audioPrompt: `An algorithm is a step-by-step procedure for solving a problem, {name}. Recipes are algorithms: specific ingredients, quantities, and ordered steps that anyone can follow to arrive at the same dish. Long division is an algorithm. Furniture assembly instructions are algorithms. Navigation directions are algorithms. Programmers write algorithms in code, but algorithms exist everywhere in non-programming life. Making an algorithm explicit has several advantages. It makes problem-solving teachable: if you can describe exactly the steps you take, you can teach those steps to someone else. It makes it automatable: if the steps are precise enough, a computer can execute them. It makes it evaluable: you can check whether the algorithm is producing the right results and identify where it breaks down. People who can articulate their algorithms, rather than just doing things intuitively, develop expertise faster and communicate their thinking more clearly.`,
            },
            {
              word: `binary search`,
              definition: `A debugging technique for narrowing down where a failure occurs: if something works at point A and fails by point Z, check the middle (point M); whichever half contains the failure, check the middle of that half. Cuts search space in half with each check. Applies to code debugging, finding errors in long documents, and any process of locating a problem in a large space.`,
              audioPrompt: `Binary search is a debugging technique for efficiently narrowing down where a failure occurs, {name}. If something works correctly at point A and fails somewhere by point Z, check the middle point M. Does the failure exist before M or after M? Whichever half contains the failure, check the middle of that half. Repeat until you've isolated exactly where the problem is. This approach cuts the search space in half with each check, which is dramatically more efficient than checking every point from A to Z in sequence. In code debugging, binary search means checking what's happening at the midpoint of a large block of code rather than stepping through line by line from the beginning. The technique applies beyond code: finding an error in a long document, locating a problem in a complex process, identifying where in a sequence something went wrong. Binary search is one of the specific techniques that makes debugging a learnable skill rather than random guessing.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            {
              word: `isolating the problem`,
              definition: `The core debugging move: precisely identifying what is failing before attempting to fix anything. When something isn't working, trying random fixes without first isolating the problem wastes time and can create new problems. The debugging discipline requires staying with the discomfort of "something is wrong and I don't know what" until the problem is located precisely.`,
              audioPrompt: `Isolating the problem is the core debugging move: precisely identifying what is failing before attempting to fix anything, {name}. When something isn't working as expected, the most common mistake is jumping immediately to fixes. You change one thing, it doesn't work. Change another. Change a third. You've now created a more complex situation than you started with. The debugging discipline reverses this: before changing anything, locate the problem precisely. What specifically is failing? Not "the code doesn't work" but "line 47 produces the wrong value when the input is negative." Not "my essay is bad" but "the third paragraph doesn't connect to the argument in the second." Not "my relationship has a recurring conflict" but "the conflict triggers specifically when one of us feels unheard about a decision." Precise location makes fixes straightforward. Vague diagnosis produces ineffective or counterproductive fixes. The emotional challenge of debugging is tolerating the discomfort of not yet knowing, which is what prevents many people from isolating before fixing.`,
            },
            {
              word: `Seymour Papert`,
              definition: `MIT computer scientist (1928-2016) who developed constructionism, the educational theory that kids learn best by building things they care about. Created LOGO, an early programming language designed for children. His work shaped maker-style learning and the idea that programming teaches computational thinking through building rather than just instruction.`,
              audioPrompt: `Seymour Papert was an MIT computer scientist who lived from 1928 to 2016 and was one of the most influential thinkers in educational technology, {name}. Papert developed constructionism, the educational theory that kids learn best by building things they care about. He created LOGO, an early programming language designed specifically for children, in the 1960s. Students wrote LOGO programs to move a "turtle" around the screen, building drawings and shapes through code. The act of building, debugging, and refining their programs taught mathematical and computational concepts more deeply than passive instruction would have. Papert's core insight: construction beats instruction for many things kids need to learn. His work was continued by Mitchel Resnick at MIT Media Lab, who created Scratch, the visual programming language now used in millions of schools worldwide. Both Papert and Resnick believed programming is valuable not because everyone needs to become a programmer, but because building programs teaches computational thinking through doing.`,
            },
          ],
        },

        {
          ...
          vocab: [
            ...
            {
              word: `Scratch`,
              definition: `Visual programming language created by Mitchel Resnick at MIT Media Lab, continuing Seymour Papert's constructionist work. Used in millions of schools worldwide. Designed to make programming accessible to children by replacing syntax with drag-and-drop blocks. Teaches computational thinking through building interactive projects rather than through instruction.`,
              audioPrompt: `Scratch is a visual programming language created by Mitchel Resnick at MIT Media Lab, {name}. Resnick continued the work of Seymour Papert, who developed constructionism — the theory that kids learn best by building things they care about. Scratch is now used in millions of schools worldwide. It makes programming accessible to children by replacing typed code syntax with drag-and-drop blocks that snap together. Students build interactive games, animations, and stories rather than learning programming concepts in the abstract. The approach embodies Papert's and Resnick's shared belief: programming is valuable not because everyone needs to become a programmer, but because building programs teaches computational thinking through doing. The act of building something, running it, finding what breaks, and fixing it teaches decomposition, pattern recognition, abstraction, algorithms, and debugging in a context where the student is motivated to learn because they care about what they're making.`,
            },
            {
              word: `natural-language prompting`,
              definition: `Interacting with AI tools using everyday language rather than programming syntax. AI tools have changed the calculation for who needs to learn to program: basic coding is now more accessible through natural-language prompting, while deep coding skills remain valuable for specific roles. Most people now benefit from computational thinking plus targeted programming only if their direction calls for it.`,
              audioPrompt: `Natural-language prompting is interacting with AI tools using everyday language rather than programming syntax, {name}. It has changed the calculation for who needs to learn to program. Previously, accessing the power of software required learning the precise syntax of programming languages. Now, AI tools can translate natural-language instructions into working code, making basic coding accessible to people without formal programming training. This doesn't eliminate the value of programming skills. Deep coding remains valuable for specific roles: building software, working with AI tools at deeper levels, data analysis, scientific research, and any work that requires customizing what computers do rather than just using what others have built. But for most people, the honest framing is: develop computational thinking through whatever means available, and pursue targeted programming skill only if your direction calls for it. Natural-language prompting has lowered the floor without eliminating the ceiling.`,
            },
          ],
        },
